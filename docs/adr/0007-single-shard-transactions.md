...
## Context
Following the implementation of MVCC and Snapshot Isolation in Sprint 6 (ADR 0006), AegisDB requires multi-operation ACID transaction processing on local shards per Master Project Plan §5, §9, §11, §14, §17, §18 & §20 (Sprint 7 / US012; Milestone M3 Gate).
...
2. **Standard State Machine:** Enforce the 4-state lifecycle `ACTIVE -> PREPARING -> PREPARED -> COMMITTED / ABORTED` to provide direct 1-phase commits while preparing hooks for Sprint 9 Two-Phase Commit (2PC).
...
- **Extensible for 2PC**: The `PREPARING`/`PREPARED` states seamlessly integrate into Sprint 9 Distributed Transactions without breaking single-shard workflows.
